...
- **Account door (web)** `[shipped]` — a dedicated `/auth` page for the Supabase account: **sign in** and **create account** with email + password, an **email-me-a-magic-link** alternative, and a **password reset** flow, all with calm, human error copy. Account & sync links to it when signed out. Signing in never touches the local journal except to merge it with the cloud. When Supabase isn't configured the page explains that the build is local-only and there is nothing to sign in to (the free tier stays account-free).
...
- **Marketing site** `[shipped]`: a four-page static site (`apps/site/`) in the app's current design language, minimal and whitespace-heavy with subtle enter-only motion (shared `site.css`, tokens synced with the app's `tokens.generated.css` including the pearlescent `--pearl` signature). The landing page has a nav, a **hero banner** (the moon-and-months art cropped from the brand's OG card, `hero-art.png`; no app icon in the hero), a clear one-paragraph pitch, a **phone demo** (an iPhone mockup that loops the Timeline with pearl-sheen month numbers, a month's journal, and Goals, with an icon-only tab bar matching the app; it fades in and rises on load with a gentle scroll parallax), a **"What you get" feature section** (four plain-language feature blocks, each with a small CSS vignette in the app's language: the Timeline, the monthly journal, goals on the calendar, and Reflect), the three calm truths, and a pricing whisper. A dedicated `/pricing` page (Vercel `cleanUrls`) has Free vs Premium cards with the $3 intro, the "never a paywall on writing" promise, and a quiet FAQ. `/privacy` and `/terms` pages (linked in every footer) carry the legal surface.
